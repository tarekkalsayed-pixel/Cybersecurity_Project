...
class RansomEyeLab:
    def __init__(self) -> None:
        ...
        self.simulator = SafeRansomwareSimulator(
            PROTECTED_FOLDER,
            DEMO_FILES_FOLDER,
            self.process_tracker,
        )
        self.monitor = ProtectedFolderMonitor(
            PROTECTED_FOLDER,
            self.handle_event,
            self.process_tracker,
        )
        # Protects status/last_recovery writes in the watchdog thread from
        # being read mid-update by Flask request threads.
    ...
    def handle_event(self, event: dict) -> None:
        # This method is called from the watchdog background thread.
        # We use self._lock whenever we read or write shared state (status,
        # last_recovery, detector internals) so Flask request threads always
            ...
            if assessment["trigger_incident"]:
                incident_payload = {
                    ...
                    "report": {
                        ...
                        "event": {key: value for key, value in event.items() if key != "dt"},
                    },
                }
                incident_id = self.logger.create_incident(incident_payload)
    ...
    def refresh_backups(self) -> dict:
        # Block refresh while an attack is in progress or files are being recovered.
        # Refreshing at that point would overwrite the clean backup with compromised files.
        with self._lock:
